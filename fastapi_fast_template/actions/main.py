...
class InitAction(ActionABC):
    ...
    def create_initial_files(self, args: ArgumentParser) -> None:
        ...
        initial_files = [
            # root
            FileBuilder(file=".fast_template.ini", build_function=self.root_content.get_fast_template_ini),
            FileBuilder(file=".gitignore", build_function=self.root_content.get_gitignore),
            FileBuilder(file=".env.sample", build_function=self.root_content.get_env_sample),
            ...
            FileBuilder(file=FileEnum.src_app, build_function=self.src_content.get_app),
            FileBuilder(file=FileEnum.src_main, build_function=self.src_content.get_main),
            FileBuilder(file=FileEnum.src_database, build_function=self.src_content.get_database),
            FileBuilder(file=FileEnum.src_repo_base, build_function=self.src_content.get_repository),
            FileBuilder(file=FileEnum.src_routers_init_, build_function=self.src_content.get_router_init),
            FileBuilder(file=FileEnum.src_utils_lifespan, build_function=self.src_content.get_lifespan),  
        ]
...
class InitActionParser(ActionParserABC):
    ...
    def parser(self):
        init = self.sub_parsers.add_parser('init', help="Initialize your project.")
